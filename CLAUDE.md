...
- [x] Phase 2: hub → RTDB (batched state/presence/events, config + commands via `/signal` since fw 0.4.0).
      **Verified on the real hub (COM7)**: steady `lastSeen` < 11 s, STOP ALL round trip about 125 ms.
      ...
      router's 2.4 GHz radio dropped out (reason 201). Hub fw 0.2.1 sends the reset reason with `HUB_BOOT`.
      2026-09-25 check: all hub restarts were reason 1 (power-on / EN / USB reset), none were crashes or
      watchdogs. The router's 2.4 GHz WiFi still drops out for about 40 s at times (beacon timeout, then
      "network not found"). The hub recovered by itself about 1 min later.
...
- [x] Phase 3: Shredder firmware (firmware/shredder, fw 0.2.1) on the real board (COM8). Hand-tested: MANUAL check/confirm/run/STOP, AUTO countdown/run/stop-when-empty, E-STOP during countdown, CANCELLED, switch flip while running, web STOP + IDENTIFY + config, power-up interlock (fixed in 0.2.1: needs OFF held 1 s). Passive buzzer wired (GPIO 13, VCC 5 V) and heard; sounds in 1.6–3.1 kHz (fw 0.2.8; a 4.25 kHz set tuned with the fw 0.2.6 pitch test sounded quieter by ear, docs/modules/shredder.md "Sounds"). Flashing this board: auto-reset doesn't work, use esptool `--before no_reset --no-stub` at 115200 and press BOOT+EN once (pressing it again mid-upload breaks the flash). fw 0.2.4: 3-way switch (default 250 ms) and START/STOP (default 50 ms) debounce adjustable from the web (software workaround until the RC filters are fitted).
